      ...
      <div className="flex flex-col items-center justify-center p-8 mt-20 text-center space-y-8 bg-[#121212]/50 border border-white/5 rounded-[3rem] max-w-2xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-400 font-medium max-w-sm mx-auto">
            A temporada ainda não começou. Prepare seus palpites para ser o primeiro a gravar seu nome no topo do {poolName}.
          </p>
        </div>
      </div>
    );
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {topThree.map((user, index) => {
          ...
          return (
            <div
              key={user.id}
              className={`relative overflow-hidden p-8 rounded-[2.5rem] border flex flex-col items-center text-center transition-all hover:scale-[1.02] duration-300 ${rank === 1
            ...
            >
              ...
              <div className="relative mb-6">
                <div className={`absolute -inset-4 rounded-full blur-2xl opacity-40 ${rank === 1 ? "bg-yellow-500" : rank === 2 ? "bg-gray-400" : "bg-amber-700"
                  }`} />
                <Avatar className={`h-24 w-24 border-4 relative z-10 ${rank === 1 ? 'border-yellow-500' : rank === 2 ? 'border-gray-400' : 'border-amber-700'
                  }`}>
                ...
                <div className={`absolute -bottom-2 -right-2 w-10 h-10 rounded-full flex items-center justify-center text-black font-black text-sm z-20 shadow-lg ${rank === 1 ? "bg-yellow-500" : rank === 2 ? "bg-gray-400" : "bg-amber-700"
                  }`}>
                  #{rank}
                </div>
              </div>
        ...
        <div className="flex flex-col divide-y divide-white/5">
          {theRest.map((user, index) => {
            ...
            return (
              <div
                key={user.id}
                className={`flex items-center justify-between p-6 hover:bg-white/5 transition-colors ${isCurrentUser ? "bg-yellow-500/5" : ""
                  }`}
              >
